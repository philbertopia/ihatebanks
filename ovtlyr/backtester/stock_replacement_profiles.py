...
_VARIANT_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    # Wheel: -0.30 delta puts, 0.20 delta calls
    "wheel_d30_c20": {
        "strategy_type": "wheel",
        "option_type": "put",
        "target_delta": -0.30,
    ...
    # Small-account proxy for the transcript's deep-ITM LEAPS idea.
    # Uses the longest liquid DTE bucket consistently present in the local cache
    # and caps the contract cost at $1,000 so one position fits the budget.
    ...
    # Practical research-only implementation of the small-account transcript thesis.
    "ditm_atr_credit_roll_small_account": {
        "target_delta": 0.80,
        "min_delta": 0.70,
        "delta_tolerance": 0.10,
        "max_extrinsic_pct": 0.20,
        ...
        "atr_roll_enabled": True,
        "atr_window": 14,
        "atr_roll_multiple": 1.0,
        "atr_roll_credit_only": True,
    },
    # ── Swing Call Variants (1-3 day directional long calls) ──────────────────
    # These variants use short-DTE options (7-21 days) with lower delta targets
    # to capture 1-3 day directional moves. Weekly expirations are required in
    # the Parquet cache (run: python main.py generate after this update).
    #
    # swing_rsi_bounce: Enter on RSI(14) < 35 oversold readings — mean-reversion
    # swing_momentum_breakout: Enter on price breaking above 5-day high — momentum
    # swing_aggressive: Both signals required, tighter DTE window, higher targets
    # swing_conservative: RSI-only, lower delta (cheaper premium), tight stop
    "swing_rsi_bounce": {
        "strategy_type": "swing_call",
        "option_type": "call",
        "target_delta": 0.60,
        "min_delta": 0.50,
        "delta_tolerance": 0.10,
        "max_extrinsic_pct": 0.60,
        "ideal_extrinsic_pct": 0.40,
        "min_open_interest": 200,
        "require_open_interest": False,
        "max_spread_pct": 0.12,
        ...
        # RSI oversold bounce entry gate
        "swing_rsi_enabled": True,
        "swing_rsi_period": 14,
        "swing_rsi_entry_max": 35.0,
        # No momentum breakout gate
        "swing_momentum_breakout_enabled": False,
        # Exits: 35% profit target, 20% stop, 3-day max hold
        "swing_profit_target_pct": 35.0,
        "swing_stop_loss_pct": 20.0,
        "swing_max_hold_days": 3,
        # Position sizing
        "max_positions": 5,
    },
    "swing_momentum_breakout": {
        "strategy_type": "swing_call",
        "option_type": "call",
        "target_delta": 0.65,
        "min_delta": 0.55,
        "delta_tolerance": 0.10,
        "max_extrinsic_pct": 0.55,
        "ideal_extrinsic_pct": 0.35,
        "min_open_interest": 200,
        "require_open_interest": False,
        "max_spread_pct": 0.12,
        "min_dte": 7,
        ...
        "avoid_final_week": False,
        # No RSI gate
        "swing_rsi_enabled": False,
        # Momentum breakout: enter when today's price > prior 5-day high
        "swing_momentum_breakout_enabled": True,
        "swing_breakout_lookback_days": 5,
        # Exits: 40% profit target, 22% stop, 3-day max hold
        "swing_profit_target_pct": 40.0,
        "swing_stop_loss_pct": 22.0,
        "swing_max_hold_days": 3,
        "max_positions": 5,
    },
    "swing_aggressive": {
        ...
        "monthly_only": False,
        "avoid_final_week": False,
        # Both signals required: RSI oversold AND 5-day high breakout
        "swing_rsi_enabled": True,
        "swing_rsi_period": 14,
        "swing_rsi_entry_max": 40.0,
        "swing_momentum_breakout_enabled": True,
        "swing_breakout_lookback_days": 5,
        # Exits: 50% profit target, 25% stop, 2-day max hold
        "swing_profit_target_pct": 50.0,
        "swing_stop_loss_pct": 25.0,
    ...
    "swing_conservative": {
        ...
        "min_delta": 0.45,
        "delta_tolerance": 0.10,
        "max_extrinsic_pct": 0.65,
        "ideal_extrinsic_pct": 0.45,
        "min_open_interest": 200,
        "require_open_interest": False,
        "max_spread_pct": 0.10,
        "min_dte": 10,
        ...
        "monthly_only": False,
        "avoid_final_week": False,
        # RSI oversold entry gate only
        "swing_rsi_enabled": True,
        "swing_rsi_period": 14,
        "swing_rsi_entry_max": 35.0,
        "swing_momentum_breakout_enabled": False,
        # Exits: 30% profit target, 15% stop (tighter), 3-day max hold
        "swing_profit_target_pct": 30.0,
        "swing_stop_loss_pct": 15.0,
        "swing_max_hold_days": 3,
        "max_positions": 8,
    },
    # ── Gap Momentum variants (2026-03) — YouTube video strategy ──────────────
    # Enter ITM calls on gap-up days: stock closes >1.5% above prior close.
    # Delta ~0.55 (slightly ITM), 21-45 DTE, 80% profit target, 5-day max hold.
    "gap_momentum_call": {
        "strategy_type": "swing_call",
        "option_type": "call",
        "target_delta": 0.55,
        ...
        "require_symbol_bullish_trend": False,
        "sit_in_cash_when_bearish": False,
        # Gap signal: enter when close > prior close by >= 1.5%
        "swing_gap_enabled": True,
        "swing_gap_min_pct": 1.5,
        "swing_gap_direction": "up",
        # No RSI or breakout gate
        "swing_rsi_enabled": False,
        "swing_momentum_breakout_enabled": False,
        # Exits: 80% profit target (video: "double your money"), 35% stop, 5-day hold
        "swing_profit_target_pct": 80.0,
        "swing_stop_loss_pct": 35.0,
        "swing_max_hold_days": 5,
        "max_positions": 5,
    },
    # Enter ITM puts on gap-down days: stock closes >1.5% below prior close.
    "gap_momentum_put": {
        "strategy_type": "swing_call",
        "option_type": "put",
        "target_delta": -0.55,
        ...
        "swing_gap_enabled": True,
        "swing_gap_min_pct": 1.5,
        "swing_gap_direction": "down",
        # No RSI or breakout gate
        "swing_rsi_enabled": False,
        "swing_momentum_breakout_enabled": False,
        # Exits: 80% profit target, 35% stop, 5-day hold
        "swing_profit_target_pct": 80.0,
        "swing_stop_loss_pct": 35.0,
        "swing_max_hold_days": 5,
        "max_positions": 5,
